...
class Product1688Parser:
    ...
    def extract_window_context(self, html_content: str) -> Optional[Dict]:
        ...
        try:
            # Thử dạng gán trực tiếp: window.context = { ... };
            try:
                direct = re.search(r"window\\.context\s*=\s*({[\s\S]*?});", html_content, re.DOTALL)
                if direct:
...
''')
            ...
            # Fallback: Đếm dấu ngoặc trên toàn bộ HTML từ marker
...
''')
    ...
    def normalize_offer_price_ranges(self, ranges: List[Dict]) -> List[Dict]:
        try:
            ...
            for r in ranges:
                normalized.append({
                    "beginAmount": int(r.get("beginAmount") or r.get("begin") or 0),
                    "endAmount": int(r.get("endAmount") or r.get("end") or 0),
                    "price": str(r.get("price") or ""),
                    "discountPrice": str(r.get("discountPrice") or ""),
                })
            return normalized
        except Exception as e:
            logger.error(f"Lỗi normalize offer price ranges: {e}")
            return []

    def normalize_sku_map(self, sku_map_any: Any) -> List[Dict]:
        try:
            if not sku_map_any:
                return []
            entries: List[Dict] = []
            if isinstance(sku_map_any, list):
                entries = sku_map_any
            elif isinstance(sku_map_any, dict):
                entries = list(sku_map_any.values())
            else:
                return []
            ...
            for e in entries:
                if not isinstance(e, dict):
                    continue
                normalized.append({
                    "skuId": to_str(e.get("skuId") or e.get("sku_id") or e.get("id") or e.get("SKUId")),
                    "specAttrs": to_str(e.get("specAttrs") or e.get("spec_attrs") or e.get("specAttributes") or e.get("spec") or e.get("attributes")),
                    "price": to_str(e.get("price") or e.get("promoPrice") or e.get("discountPrice") or e.get("salePrice")),
                    "canBookCount": to_str(e.get("canBookCount") or e.get("amountOnSale") or e.get("canBookingCount") or e.get("stock") or e.get("amount")),
                    "saleCount": to_str(e.get("saleCount") or e.get("soldNum") or e.get("sale_num") or e.get("sales") or 0),
                    "specId": to_str(e.get("specId") or e.get("spec_id") or e.get("idString") or e.get("specKey") or e.get("key")),
                })
            return normalized
        except Exception as e:
            logger.error(f"Lỗi normalize sku map: {e}")
            return []
    ...
    def parse_images(self, images: List[str]) -> List[Dict]:
        ...
        try:
            ...
            # Chấp nhận list[str] hoặc list[dict]
            urls: List[str] = []
            for el in images:
                if isinstance(el, str):
                    urls.append(el)
                elif isinstance(el, dict):
                    for key in ("url", "imageUrl", "imgUrl", "image", "src", "original"):
                        if key in el and isinstance(el[key], str):
                            urls.append(el[key])
                            break
        ...
        try:
            ...
            formatted_info = {
                ...
                "product": {
                    ...
                    "sku_map": self.normalize_sku_map(parsed_data.get("skuMap")),
                    "offer_price_ranges": self.normalize_offer_price_ranges(parsed_data.get("offerPriceRanges", []))
                },
                "raw_data": {
